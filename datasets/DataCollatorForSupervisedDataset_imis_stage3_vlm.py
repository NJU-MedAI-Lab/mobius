...
def DataCollatorForSupervisedDataset_stage3_vlm(list_data_dict: Sequence[Dict], inference: bool = False) -> Dict[str, torch.Tensor]:
    ...
    if max_mask_nums > 0:
        ...
        for idx, instance in enumerate(list_data_dict):
            if len(instance['gt_masks']) > 0:
                ...
                valid_mask_bool.extend([True] * len(instance['gt_masks']))
            else:
                valid_mask_bool.extend([False])
    ...
    final_batch = {
        ...
        "valid_mask_bool": batch['valid_mask_bool'],
