...
def extract_masks_fun(source, mask_root_path, pattern=r'<mask>(.*?)</mask>'):
    ...
    for idx, item in enumerate(source['conversations']):
        ...
        if mask_name_lst:
            assert len(
                mask_name_lst) == 1, "Only one mask is supported in one turns."
            ...
            if '</mask>' in pattern:
                ...
                source['conversations'][idx]['value'] = item['value'].replace(
                    f'<mask>{mask_name_lst[0]}</mask>', '')
...
def extract_masks_fun_probe(source, mask_root_path, mask_root_path2=None, pattern=r'<mask>(.*?)</mask>'):
    ...
    def find_(root_path):
        for idx, item in enumerate(source['conversations']):
            ...
            if mask_name_lst:
                assert len(
                    mask_name_lst) == 1, "Only one mask is supported in one turns."
                mask_path = os.path.join(root_path, mask_name_lst[0])
    ...
    root_paths = []
    root_paths.append(mask_root_path)
    root_paths.extend(get_subdirs(mask_root_path2))
    flag = False
...
def find_valid_root_path_for_item(source, mask_root_path, mask_root_path2=None, pattern=r'<mask>(.*?)</mask>'):
    def check_path(root_path):
        for item in source['conversations']:  # 注意：这里遍历 source 的 conversations
            mask_name_lst = re.findall(pattern, str(item['value']))
            if mask_name_lst:
                # assert len(mask_name_lst) == 1, "Only one mask is supported in one turns." # probe里有，这里可以省略或处理
                if len(mask_name_lst) != 1:
                    # 如果不符合规范，这个source本身可能就有问题，或者我们选择跳过检查，让后续加载时报错
                    # 为了和probe一致，我们按probe的逻辑，只要有一个找不到就失败
                    pass
                mask_path = os.path.join(root_path, mask_name_lst[0])
                if not os.path.isfile(mask_path):
                    return False  # 在这个 root_path 下找不到，就失败
        return True  # 在这个 root_path 下都找到了
    ...
    # 依次检查，返回第一个有效的路径
    for root_path in root_paths:
        if check_path(root_path):
            return root_path  # 找到有效路径，立即返回

    return None  # 所有路径都检查完，没找到
...
class LazySupervisedDataset_stage1(Dataset):
    ...
    def __init__(self, data_path: str,
                 ...
                 image_size=1024,
                 mode="train"):
        super(LazySupervisedDataset_stage1, self).__init__()
        ...
        self.transform_clip_nomask = transforms.Compose(
            [
                Resize(keys=["image"],
                       target_size=(self.clip_img_size, self.clip_img_size)),
                PermuteTransform(keys=["image"], dims=(2, 0, 1)),
        ...
        for i, item in enumerate(tqdm(list_data_dict[data_key], desc="Loading and validating data")):
            # --- 图片路径处理保持不变 ---
            if 'image' in item:
                ...
                if os.path.exists(image_file):
                    ...
                    image_path = os.path.join(
                        '/'.join(image_folder.split('/')[:-1]), image_file)
                else:
                    # 修复原代码中判断扩展名的逻辑
                    if image_file[-4:] != '.png' and image_file[-4:] != '.jpg' and image_file[-5:] != '.jpeg':  # 更健壮的判断
                        # 或者使用 os.path.splitext(image_file)[1]
                        image_path = os.path.join(
                            image_folder, image_file + '.png')  # 默认加.png
                    else:
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        sources = {k: v for k, v in item.items() if
                   k != 'imask'}
        sources = [sources]  # 包装成列表以匹配 extract_masks_fun 的期望输入

        answer_type = item.get('answer_type', 'open')  # 从原始 item 获取
        ...
        try:
            if 'image' in sources[0] and 'label' in sources[0]:
                # 获取label
                ...
                #     '<region></region>', '')  # 先不加region

                if "<region>" in item['conversations'][0]['value'] and "</region>" in item['conversations'][0]['value']:
                    region_masks = [
                        clip_inputs['mask'].squeeze(0)]  # list[H,W]
                # [3,clip_img_size,clip_img_size]
                image_clip = clip_inputs['image']

                # 对region masks下采样
                region_masks = [
                    cv2.resize(np.array(region_mask), None, fx=1 / 2,
                               fy=1 / 2, interpolation=cv2.INTER_NEAREST)
                    for region_mask in region_masks
                ]
                # 生成粗糙的mask，模拟弱标注输入
                region_masks, is_valid_region = generate_mask_with_sub_component(region_masks, min_area=0.2, max_area=1,
                                                                                 min_thresh=10)
                ...
                data_dict = preprocess(
                    sources,
                    self.tokenizer,
                    has_image=('image' in item))  # 检查原始数据

            # 只有image,没有label
            elif 'image' in sources[0] and 'label' not in sources[0]:
                # image_path = self.list_data_dict[i]['image'] # 从原始 item 获取
                image_path = item['image']  # 更直接
                processor = self.data_args.image_processor
                ...
                clip_inputs_ori = {'image': image_rgb}
                clip_inputs = self.transform_clip_nomask(clip_inputs_ori)
            ...
            if 'image' in item:  # 检查原始数据
                ...
                data_dict['masks'] = gt_masks  # size和原始图像的size一致
            ...
            # 如果有region_masks但不是有效区域，将labels设为无效，并用一个假区域来占位
