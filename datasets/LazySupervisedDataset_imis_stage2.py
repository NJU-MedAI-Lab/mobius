...
from utils.utils import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
...
class LazySupervisedDataset_stage2(Dataset):
    ...
    def __init__(self, data_path: str,
                 ...
                 image_size=1024,
                 mode='train'):
        super(LazySupervisedDataset_stage2, self).__init__()
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        try:
            ...
            if 'image' in sources[0]:
                ...
                if "</region>" in item["conversations"][-1]["value"]:
                    region_masks = [
                        clip_inputs['mask'].squeeze(0)]  # list[H,W]
                    # 对region masks下采样
                    region_masks = [
                        cv2.resize(np.array(region_mask), None, fx=1 / 4,
            ...
            region_masks = [process_mask(mask) for mask in region_masks]
            ...
            if len(region_masks) > 0 and not is_valid_region:
                data_dict['labels'] = torch.zeros_like(
                    data_dict["labels"])
