    ...
    def __call__(self, data):
        ...
        for key in self.keys:
            if len(d[key].shape) == 4:
                label = d[key]
                resized_labels = np.zeros(
                    (label.shape[0], self.target_size[0], self.target_size[1]))
                for i in range(label.shape[0]):
                    pil_label = to_pil_image(label[i])
                    resized_label = resize(
                        pil_label, self.target_size, interpolation=InterpolationMode.NEAREST)
                    resized_labels[i] = np.array(resized_label)
                d[key] = resized_labels
            else:
                image = to_pil_image(d[key])
                d[key] = resize(image, self.target_size,
...
def get_points_from_mask(mask, get_point=1, top_num=0.5):
    ...
    fg_coords = np.argwhere(mask == 1)[:, ::-1]  # 前景坐标
    bg_coords = np.argwhere(mask == 0)[:, ::-1]  # 背景坐标

    centroid = np.mean(fg_coords, axis=0)  # 计算前景质心
    distances = np.sqrt(
        np.sum((fg_coords - centroid)**2, axis=1))  # 计算前景点到质心的距离
    sorted_indices = np.argsort(distances)  # 将距离从小到大排序
    num_points = len(fg_coords)
    top_k = int(num_points * top_num)
    top_indices = sorted_indices[:top_k]  # 选距离前50%近的点作为候选点

    # 如果有前景点，就在前景候选点中随机选一个，没有就在背景点中随机选一个
    try:
    ...
    coords, labels = torch.as_tensor(
        [coord.tolist()], dtype=torch.float), torch.as_tensor([label], dtype=torch.int)
...
def get_bboxes_from_mask(masks, offset=0):
    ...
    for i in range(B):
        mask = masks[i]
        y_coords, x_coords = torch.nonzero(mask, as_tuple=True)  # 选出前景点

        if len(y_coords) == 0 or len(x_coords) == 0:
            bounding_boxes.append((0, 0, 0, 0))  # 没有前景点bbox设为全0
        else:
            y0, y1 = y_coords.min().item(), y_coords.max().item()  # 前景最上和最下坐标
            x0, x1 = x_coords.min().item(), x_coords.max().item()  # 前景最左和最右坐标

            if offset > 0:  # 如果设置了offset
                y0 = max(0, y0 + torch.randint(-offset, offset + 1, (1,)).item())
                y1 = min(W - 1, y1 + torch.randint(-offset,
                         offset + 1, (1,)).item())
                x0 = max(0, x0 + torch.randint(-offset, offset + 1, (1,)).item())
                x1 = min(H - 1, x1 + torch.randint(-offset,
                         offset + 1, (1,)).item())
...
if __name__ == '__main__':
    ...
    for box in bboxes:
        x0, y0, x1, y1 = box
        plt.gca().add_patch(plt.Rectangle((x0, y0), x1-x0,
                                          y1-y0, edgecolor='r', facecolor='none'))
