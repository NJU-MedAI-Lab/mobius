...
import numpy as np
from model.IMIS.dataloaders.data_utils import get_points_from_mask, get_bboxes_from_mask
...
class IMISNet(nn.Module):
    def __init__(
        ...
        self.text_out_dim = sam.text_out_dim  # nn.Linear(512, 768)
        if tokenizer is None:
            self.tokenizer = AutoTokenizer.from_pretrained(
                '/workspace/EviMed/huggingface/clip-vit-base-patch32/')
            print('Load default tokenizer from clip-vit-base-patch32')
        else:
            self.tokenizer = tokenizer
            print('Load tokenizer from args')
    ...
    def image_forward(self, image):  # 对每一张图片，根据图片上mask的数量，复制image_embedding
        img_shape = image.shape
        image_embedding = self.image_encoder(image)
        assert len(
            image_embedding.shape) == 4, f'required shape is (B, C, H, W), but we get {image_embedding.shape}'
        ...
        if self.test_mode:
            ...
            for bs in range(img_shape[0]):
                image_embed = image_embedding[bs].repeat(
                    self.select_mask_num, 1, 1, 1)
                image_embeddings_repeat.append(image_embed)
            return_img_embed = torch.cat(
                image_embeddings_repeat, dim=0).to(image_embedding.device)
    ...
    def forward_decoder(self, image_embedding, prompt):
        ...
        )  # 用sam的prompt_encoder对输入的prompt进行编码

        outputs = self.mask_decoder(
            image_embeddings=image_embedding,
            image_pe=self.prompt_encoder.get_dense_pe(),  # 特征图对应的位置编码
            text_prompt_embeddings=prompt.get("text_inputs", None),
            sparse_prompt_embeddings=sparse_embeddings,  # for points and boxes prompts
            dense_prompt_embeddings=dense_embeddings,  # for mask prompts
            multimask_output=self.multimask_output,  # 是否支持多mask输出，即输出多个masks
        )

        # 无论是否是multimask，都只输出一个mask
        if self.multimask_output:
            low_res_masks, iou_pred, semantic_pred = self.get_max_pred(outputs)
        else:
            low_res_masks, iou_pred, semantic_pred = outputs[
                'low_res_masks'], outputs['iou_pred'], outputs['semantic_pred']

        # 将低分辨率mask上采样
        masks = F.interpolate(
            low_res_masks, size=self.image_size, mode='bilinear', align_corners=False)
    ...
    def supervised_prompts(self, classes, labels, pred_masks, low_res_masks, specify_prompt):
        ...
        if specify_prompt == 'points':
            bs = labels.shape[0]
            bs_prompts.update(
                self.process_points_prompt(bs, labels, pred_masks))
    ...
    def unsupervised_prompts(self, pseudo_labels, pred_masks, low_res_masks, specify_prompt):
        ...
        if specify_prompt == 'points':
            ...
            bs_prompts.update(self.process_points_prompt(
                bs, pseudo_labels, pred_masks))
    ...
    def process_points_prompt(self, bs, labels, pred_masks=None):
        ...
        bs_point_coords = torch.empty(
            (bs, point_num, 2), dtype=torch.long, device=labels.device)
        bs_point_labels = torch.empty(
            (bs, point_num), dtype=torch.long, device=labels.device)

        for idx in range(bs):
            if pred_masks is None:
                point_coords, point_labels = get_points_from_mask(
                    labels[idx], get_point=1)
            else:
                point_coords, point_labels = self.get_points_from_interaction(
                    error_area[idx], pred_masks[idx], labels[idx], get_point=point_num)

            bs_point_coords[idx, :] = torch.as_tensor(
                point_coords, device=labels.device)
            bs_point_labels[idx, :] = torch.as_tensor(
                point_labels, device=labels.device)
    ...
    def text_tokenizer(self, text, tamplate='A segmentation area of a {}.'):
        ...
        tokens = self.tokenizer(text_list, padding=True,
                                return_tensors="pt")  # 使用clip tokenizer分词
        for key in tokens.keys():
            tokens[key] = tokens[key].to(self.device)
        text_outputs = self.text_model(**tokens)  # sam text model
    ...
    def load_category_weights(self, src_weights=None):
        if src_weights is not None:
            with open(src_weights, "rb") as f:
                self.src_weights, self.categories_map, self.category_to_index, self.index_to_category = pickle.load(
                    f)
                self.src_weights = torch.tensor(
                    self.src_weights).to(self.device)
    ...
    def category_loss(self, semantic_preds, classes, ce_loss):
        ...
        logits = nn.functional.normalize(
            semantic_preds, dim=-1) @ self.src_weights
    ...
    def get_max_pred(self, outputs):
        ...
        low_mask_indices = max_indices.unsqueeze(
            -1).unsqueeze(-1).expand(-1, -1, low_res_masks.shape[2], low_res_masks.shape[3])
        semantic_idices = max_indices.unsqueeze(-1).expand(-1, -1, 512)
        low_res_masks_selected = torch.gather(
            low_res_masks, 1, low_mask_indices)
    ...
    def get_points_from_interaction(self, error, pr, gt, get_point=1):
        ...
        if indices.shape[0] > 0:
            selected_indices = indices[np.random.choice(
                indices.shape[0], get_point, replace=True)]
        else:
