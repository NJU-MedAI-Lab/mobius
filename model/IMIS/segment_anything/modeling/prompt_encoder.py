...
class PromptEncoder(nn.Module):
    ...
    def _embed_points(
        ...
        point_embedding = self.pe_layer.forward_with_coords(
            points, self.input_image_size)  # B,N+1,256
        point_embedding[labels == -1] = 0.0

        self.not_a_point_embed.weight = torch.nn.Parameter(
            self.not_a_point_embed.weight.to(point_embedding.dtype), requires_grad=True)  # todo
        self.point_embeddings[0].weight = torch.nn.Parameter(
            self.point_embeddings[0].weight.to(point_embedding.dtype), requires_grad=True)  # todo
        self.point_embeddings[1].weight = torch.nn.Parameter(
            self.point_embeddings[1].weight.to(point_embedding.dtype), requires_grad=True)  # todo
    ...
    def forward(
        ...
        sparse_embeddings = torch.empty(
            (bs, 0, self.embed_dim), device=self._get_device())  # B,0,256  空[]
        ...
        if text is not None:
            sparse_embeddings = torch.cat(
                [sparse_embeddings, text.unsqueeze(1)], dim=1)
...
class PositionEmbeddingRandom(nn.Module):
    ...
    def _pe_encoding(self, coords: torch.Tensor) -> torch.Tensor:
        ...
        # todo
        coords = coords @ self.positional_encoding_gaussian_matrix.to(
            torch.float32)
