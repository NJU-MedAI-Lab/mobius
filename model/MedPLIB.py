...
from .mobius.model.language_model.medplib_moe_llama import (EvalMedPLIBMoELlamaForCausalLM,
...
class MedPLIBForCausalLM(MedPLIBMoELlamaForCausalLM):
    def __init__(
        ...
        **kwargs,
        ...
        if not kwargs.get('test_only', False):
            ...
            config.moe["router_aux_loss_coef"] = kwargs.get(
                "router_aux_loss_coef")
            config.moe["eval_capacity_factor"] = kwargs.get(
    ...
    def expand_embedding(self, image_embeddings, valid_mask_bool):
        ...
        for i, factor in enumerate(expand_factors):
            if factor > 0:
                expanded_tensor = torch.index_select(
                    image_embeddings, 0, torch.tensor([i]).cuda())
    ...
    def forward(self, **kwargs):
        ...
        return self.model_forward_v_l(**kwargs)

    def model_forward_v_l(
            self,
        ...
        if seg_flag:
            ...
            image_embeddings = self.expand_embedding(
                image_embeddings, valid_mask_bool)
            ...
            seg_token_mask = torch.cat(
                [torch.zeros((seg_token_mask.shape[0], 575)
                             ).bool().cuda(), seg_token_mask],
                dim=1,
            )
        ...
        last_hidden_state = self.model.text_hidden_fcs[0](
        ...
        for batch_idx in range(len(pred_masks)):
            ...
            mask_iou_loss += self.iou_loss_fn(pred_mask, gt_mask,
                                              pred_ious[batch_idx]) * gt_mask.shape[0]
            mask_focal_loss += self.focal_loss_fn(
                pred_mask, gt_mask) * gt_mask.shape[0]
        ...
        unscale_mask_loss = unscale_mask_bce_loss + unscale_mask_dice_loss + \
            unscale_mask_iou_loss + unscale_mask_focal_loss
    ...
    def evaluate(
        ...
        with torch.no_grad():
            ...
            all_last_layer_hidden_states = [
                output_hidden_states[i][-1] for i in range(len(output_hidden_states))]
            output_hidden_states = torch.cat(
                all_last_layer_hidden_states, dim=1)
            ...
            if torch.sum(seg_token_mask) == 0 and inference_demo:
                ...
                seg_token_mask = torch.cat(
                    [
                        torch.zeros(
                            (seg_token_mask.shape[0], 575)).bool().cuda(),
                        seg_token_mask,
                    ],
                    dim=1,
                )
                ...
                if isinstance(output_hidden_states, tuple):
                    hidden_states.append(
                        self.model.text_hidden_fcs[0](output_hidden_states[-1]))
                else:
                    hidden_states.append(
                        self.model.text_hidden_fcs[0](output_hidden_states))

                last_hidden_state = torch.stack(
                    hidden_states, dim=-1).sum(dim=-1)
                ...
                # use the last hidden state when there is no seg tokens
                    ...
                    pred_embeddings = last_hidden_state[:1, -
                                                        2:-1, :].squeeze(1)
                ...
                for i in range(len(pred_embeddings)):
                    ...
                    (
                        ...
                        boxes=None,
                        masks=None,
                        text_embeds=pred_embeddings[i].unsqueeze(
                            0).unsqueeze(1),
                    )
    ...
    def postprocess_masks(self, masks: torch.Tensor, input_size: Tuple[int, ...], original_size: Tuple[int, ...]):
        ...
        masks = masks[:, :, pad_top:pad_top+original_height,
