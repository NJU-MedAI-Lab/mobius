...
def parse_option():
    ...
    parser.add_argument(
        '--pred', type=str, default="/workspace/EviMed/IMIS_runs/imis-stage2-full-4620/infer_res/BTCV/qa_dataset.jsonl", help='path to prediction file', )
    parser.add_argument(
        '--save', type=str, default="/workspace/EviMed/IMIS_runs/imis-stage2-full-4620/infer_res/BTCV/", help='save path', )
    parser.add_argument('--candidate_set', type=str,
                        default=None, help='path to prediction file', )
...
def evaluate(test_dict_lst, args, criterion=None):
    ...
    if eval_closed:
        closed_score = sum(closed_scores['hit']) / len(
            closed_scores['hit']) if len(closed_scores['hit']) != 0 else 0.0
    else:
        closed_score = 0.0
