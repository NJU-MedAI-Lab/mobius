...
def parse_option():
    ...
    parser.add_argument(
        '--pred', type=str, default="/workspace/EviMed/IMIS_tests/llava-v1.5-7b/openvqa/test_openVQA.jsonl", help='path to prediction file', )
    parser.add_argument(
        '--save', type=str, default="/workspace/EviMed/IMIS_tests/llava-v1.5-7b/openvqa/", help='save path', )
...
def evaluate(test_dict_lst, args, criterion=None):
    ...
    modality_mapping = json.load(
        open('/workspace/EviMed/IMIS_data_jsons/modality_mapping.json', 'r'))
    ...
    for item in tqdm(test_dict_lst):
        ...
        gt_value = gt_value.replace('<SEG>', '').replace('</SEG>', '').strip()
        ...
        if answer_type in ['open', 'other', 'number']:
            ...
            if modality not in open_scores_modal_dict:
                open_scores_modal_dict[modality] = collections.defaultdict(
                    list)
            ...
            open_scores_modal_dict[modality]['hit'].append(
                calculate_exactmatch(pred_value, gt_value))
            open_scores_modal_dict[modality]['f1'].append(f1_score)
            open_scores_modal_dict[modality]['precision'].append(precision)
            open_scores_modal_dict[modality]['recall'].append(recall)
            open_scores_modal_dict[modality]['q_id'].append(
                item['question_id'])
    ...
    if eval_open:
        ...
        for k in open_scores_modal_dict:
            open_scores_modal_dict_final[k] = {}
            open_scores_modal_dict_final[k]['exact'] = sum(
                open_scores_modal_dict[k]['hit']) / len(open_scores_modal_dict[k]['hit'])
            open_scores_modal_dict_final[k]['f1'] = sum(
                open_scores_modal_dict[k]['f1']) / len(open_scores_modal_dict[k]['f1'])
            open_scores_modal_dict_final[k]['precision'] = sum(
                open_scores_modal_dict[k]['precision']) / len(open_scores_modal_dict[k]['precision'])
            open_scores_modal_dict_final[k]['recall'] = sum(
                open_scores_modal_dict[k]['recall']) / len(open_scores_modal_dict[k]['recall'])
            open_scores_modal_dict_final[k]['count'] = len(
                open_scores_modal_dict[k]['q_id'])
    else:
        exact_score = 0.0
        f1_score = 0.0
        precision = 0.0
...
if __name__ == '__main__':
    ...
    # perform evaluation
    results, open_scores_modal_dict_final = evaluate(
        test_dict_lst, args)
    print(results)
    print(open_scores_modal_dict_final)

    # 写入txt文件
    with open(os.path.join(args.save, "metrics.txt"), "w", encoding="utf-8") as f:
        f.write(results)
        f.write("\n")
        f.write(str(open_scores_modal_dict_final))
