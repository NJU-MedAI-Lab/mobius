...
def parse_option():
    ...
    parser.add_argument(
        '--pred', type=str, default="/workspace/EviMed/MedPLIB_tests/MedPLIB-7b-2e/grounding/test_openVQA.jsonl", help='path to prediction file', )
    parser.add_argument(
        '--save', type=str, default="/workspace/EviMed/MedPLIB_tests/MedPLIB-7b-2e/grounding/", help='save path', )
...
def evaluate(test_dict_lst, args, criterion=None):
    ...
    closed_cnt = 0
    ori_data = json.load(
        open("/workspace/EviMed/IMIS_data_jsons/test_openVQA.json", 'r'))['test']
    ori_dict = {
        (item['image'], item['class']): item['conversations'][-1]['value']
        for item in ori_data
    }
    for item in tqdm(test_dict_lst):
        image_path = item['image_path']
        class_name = item['class']
        gt_value = ori_dict[(image_path, class_name)]
        gt_value = gt_value.replace('<SEG>', '').replace('</SEG>', '').strip()
        pred_value = item['vlm_output'].lower()
...
if __name__ == '__main__':
    ...
    # 写入txt文件
    with open(os.path.join(args.save, "openvqa_metrics.txt"), "w", encoding="utf-8") as f:
        f.write(results)
