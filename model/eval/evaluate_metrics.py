...
def calculate_appearance_with_normalization(prediction, reference, candidate_set):
    ...
    for candidate_answer in candidate_set:
        ...
        similarity_list.append(similarity_candidate_prediction(
            candidate_answer, prediction_words))

    final_prediction = candidate_answer_normalized_list[argmax(
        similarity_list)]
    ...
    if final_prediction == reference:
        ...
        # print(prediction, '---', reference, '---', final_prediction)
        return 0.0
...
def calculate_f1score(candidate, reference):
    ...
    # print('reference_words', reference_words)
    word_set = set()
    for word in candidate_words:
        word_set.add(word)
    for word in reference_words:
