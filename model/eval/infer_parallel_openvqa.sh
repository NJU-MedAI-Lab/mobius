...
####################data config################
### For BTCV test
ROOT_PATH="/workspace/EviMed/IMIS_data_jsons"
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    ...
    --pretrain_mm_mlp_adapter="/workspace/EviMed/huggingface/llava-llama-2-7b-chat-clip336-pretrain-medplib-stage1/mm_projector.bin" \
    ...
    --answers-file /workspace/EviMed/Mobius_tests/$NAME/$DATA_TYPE/$SPLIT/${CHUNKS}_${IDX}.jsonl \
    --workers 12 \
    --out_dim 768 \
    --eval_vqa \
    --num-chunks $CHUNKS \
...
output_file=/workspace/EviMed/Mobius_tests/$NAME/$DATA_TYPE/$SPLIT.jsonl
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    cat /workspace/EviMed/Mobius_tests/$NAME/$DATA_TYPE/$SPLIT/${CHUNKS}_${IDX}.jsonl >> "$output_file"
done