...
from datasets import LazySupervisedDataset_test, DataCollatorForSupervisedDataset_test
...
def parse_args(args):
    ...
    parser.add_argument('--eval_seg', action='store_true',
                        default=False, help='')
    parser.add_argument('--eval_vqa', action='store_true',
                        default=False, help='')
    ...
    parser.add_argument('--vis_mask', action='store_true',
                        default=False, help='')
    parser.add_argument('--save_seperate', action='store_true',
                        default=False, help='')
...
def main(args):
    ...
    model = MobiusForCausalLM.from_pretrained(
        args.version, tokenizer=tokenizer, torch_dtype=torch_dtype, low_cpu_mem_usage=False, ignore_mismatched_sizes=False, **model_args
    )
    ...
    val_dataset = LazySupervisedDataset_test(
        args.val_data_path, tokenizer, data_args, args.sam_img_size, mode='test')
    ...
    if val_dataset is not None:
        ...
        val_loader = torch.utils.data.DataLoader(
            ...
            drop_last=False,
            collate_fn=partial(
                DataCollatorForSupervisedDataset_test,
                inference=True,
            ),
        )
    ...
    model.eval()
    if args.eval_seg:
        validate_seg(val_loader, model, 0, args,
                     tokenizer)
        exit()
    if args.eval_vqa:
...
def vis_overlay_masks(original_image_path, prediction_mask, ground_truth_mask, save_path, save_seperate=False):
    ...
    prediction_overlay_image = original_image.copy()
    ground_truth_overlay_image = original_image.copy()
    mask = prediction_overlay > 0
    mask_truth = ground_truth_overlay > 0
    prediction_overlay_image = np.where(
        mask,
        cv2.addWeighted(original_image, 0.5, prediction_overlay, 0.9, 0),
        original_image
    )
    ground_truth_overlay_image = np.where(
        mask_truth,
        cv2.addWeighted(original_image, 0.5, ground_truth_overlay, 0.9, 0),
        original_image
    )
    ...
    combined_image = np.concatenate(
        [original_image, ground_truth_overlay_image, prediction_overlay_image], axis=1)
    ...
    if save_seperate:
        cv2.imwrite(save_path.replace(".png", "_original.png"),
                    cv2.cvtColor(original_image, cv2.COLOR_RGB2BGR))
        cv2.imwrite(save_path.replace(".png", "_ground_truth.png"),
                    cv2.cvtColor(ground_truth_overlay_image, cv2.COLOR_RGB2BGR))
        cv2.imwrite(save_path.replace(".png", "_prediction.png"),
                    cv2.cvtColor(prediction_overlay_image, cv2.COLOR_RGB2BGR))
...
def validate_vqa(val_loader, model_engine, epoch, args, tokenizer, fea_hooks=None):
    ...
    for input_dict in pbar:
        ...
        modality = input_dict["modality_list"][0]
...
def validate_seg(val_loader, model_engine, epoch, args, tokenizer, fea_hooks=None):
    ...
    for input_dict in pbar:
        ...
        if modality not in modality_metric.keys():
            ...
            modality_metric[modality]['count'] = 0

        class_name = input_dict["class_list"][0]
        if class_name not in class_metric.keys():
            class_metric[class_name] = {}
            class_metric[class_name]['iou'] = []
            class_metric[class_name]['dice'] = []
            class_metric[class_name]['count'] = 0
        ...
        with torch.no_grad():
            ...
            outputs, pred_masks, gt_masks = model_engine.evaluate(
                **input_dict,
                max_new_tokens=1024,
            )
        ...
        modality_metric[modality]['count'] += 1

        class_metric[class_name]['iou'].append(iou)
        class_metric[class_name]['dice'].append(dice)
        class_metric[class_name]['count'] += 1
        ...
        if args.vis_mask:
            ...
            save_path = os.path.join(
                args.vis_save_path, img_name+'iou'+str(round(iou, 4))+'.png')
            os.makedirs(os.path.dirname(save_path), exist_ok=True)
            vis_overlay_masks(
                input_dict['image_paths'][0], output_i, mask_i, save_path, args.save_seperate)
    ...
    print("miou: {:.6f}, mDice: {:.6f}\n".format(miou, mDice))

    modality_metric_res = {}
    for modality in modality_metric.keys():
        modality_metric_res[modality] = {}
        modality_metric_res[modality]['iou'] = round(
            sum(modality_metric[modality]['iou'])/len(modality_metric[modality]['iou']), 6)
        modality_metric_res[modality]['dice'] = round(
            sum(modality_metric[modality]['dice'])/len(modality_metric[modality]['dice']), 6)
        modality_metric_res[modality]['count'] = modality_metric[modality]['count']
    print("Metrics per modality:\n")
    print(modality_metric_res)

    class_metric_res = {}
    for class_name in class_metric.keys():
        class_metric_res[class_name] = {}
        class_metric_res[class_name]['iou'] = round(
            sum(class_metric[class_name]['iou'])/len(class_metric[class_name]['iou']), 6)
        class_metric_res[class_name]['dice'] = round(
            sum(class_metric[class_name]['dice'])/len(class_metric[class_name]['dice']), 6)
        class_metric_res[class_name]['count'] = class_metric[class_name]['count']
    print("Metrics per class:\n")
    print(class_metric_res)
    return miou, mDice
