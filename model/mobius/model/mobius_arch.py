...
def point_sample(input, point_coords, return_dtype, **kwargs):
    ...
    output = F.grid_sample(
        input.float(), (2.0 * point_coords - 1.0).float(), **kwargs)
...
class LlavaMetaModel:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        if pretrain_mm_mlp_adapter is not None:
            print(
                f"Loading pretrain mm mlp adapter from {pretrain_mm_mlp_adapter}")
            ...
            print('load pretrain mm mlp projector successfully!!!')
        else:
            print('no pretrain mm mlp adapter!!!')
            # self.mm_projector = build_vision_projector(self.config)
            # print('initialize mm mlp projector successfully!!!')
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        self, input_ids, attention_mask, past_key_values, labels, images, region_masks, valid_region_masks_bool, is_embeds=False
    ):
        ...
        if region_flag:
            valid_region_masks_bool = torch.tensor(
                [any(item) for item in valid_region_masks_bool])
            region_feature_map = region_feature_map[valid_region_masks_bool]

            if is_embeds:
                region_features = [region_mask for region_mask in region_masks]
            else:
                if region_geo_sampler:
                    # pdb.set_trace()
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            while image_token_indices.numel() > 0:
                ...
                if region_flag:
                    assert (cur_input_ids[:image_token_start]
                            == REGION_TOKEN_INDEX).sum() == 0
            ...
            if cur_input_ids.numel() > 0:
                ...
                region_indices = (cur_input_ids == REGION_TOKEN_INDEX).nonzero(
                    as_tuple=True)[0].tolist()
                cur_input_ids = cur_input_ids[cur_input_ids !=
                                              REGION_TOKEN_INDEX]
                ...
                if region_flag and valid_region_masks_bool[batch_idx] is not None:
                    for idx, indice in enumerate(region_indices):
                        region_features_idx = torch.sum(
                            valid_region_masks_bool[:batch_idx+1]).item() - 1
                        text_input_embeds = torch.cat(
                            (text_input_embeds[:indice], region_features[region_features_idx][idx].unsqueeze(0), text_input_embeds[indice:]))
                else:
                    if region_flag:
            ...
            for cur_new_embed in new_input_embeds:
                cur_new_embed = torch.cat(
                    (
                        ...
                        torch.zeros(
                            (max_len -
                             cur_new_embed.shape[0], cur_new_embed.shape[1]),
    ...
    def extract_region_feature(self, region_feature_map, region_masks, original_dtype, return_dtype):
        all_region_features = []
        assert len(region_feature_map) == len(
            region_masks), f'{len(region_feature_map)}, {len(region_masks)}'
        ...
        for region_feature_map_i, region_masks_list_i in zip(region_feature_map, region_masks):
            if len(region_masks_list_i) == 0:
                ...
                ori_image_wh = torch.tensor(
                    [region_masks_list_i[0].shape[0], region_masks_list_i[0].shape[1]], device=region_masks_list_i[0].device)[None,]
                # list of elements of shape [num_sample_point, 2]
                non_zero_pos = [rand_sample((m.nonzero(
                )/ori_image_wh), self.get_model().max_sample_point) for m in region_masks_list_i]
                ...
                dup_region_feature_map_i = region_feature_map_i.reshape(
                    h, w, c).permute(2, 0, 1)
                dup_region_feature_map_i = dup_region_feature_map_i.unsqueeze(
                    0).repeat(non_zero_pos.shape[0], 1, 1, 1)
                ...
                region_feature_i = point_sample(dup_region_feature_map_i_ori_type,
                                                non_zero_pos.flip(dims=(2,)).type(
                                                    original_dtype),
                                                return_dtype,
                                                align_corners=True
                                                )
                region_feature_i = region_feature_i.to(
                    dup_region_feature_map_i.dtype)
                # [num_mask, C]
                region_feature_i = torch.stack([x[m].mean(dim=0) for x, m in zip(
                    region_feature_i.transpose(1, 2), non_zero_pos_mask)]).nan_to_num()
