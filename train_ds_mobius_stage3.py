...
from datasets import (DataCollatorForSupervisedDataset_stage3_vlm, LazySupervisedDataset_stage3_vlm,
                      DataCollatorForSupervisedDataset_stage3_sam, LazySupervisedDataset_stage3_sam,
...
def parse_args(args):
    ...
    parser.add_argument("--sam_ce_loss_weight", default=5.0, type=float)
    parser.add_argument("--vlm_mask_loss_weight", default=1.0, type=float)
    parser.add_argument("--sam_mask_loss_weight", default=2.0, type=float)
...
def main(args):
    ...
    if args.local_rank == 0:
        for name, param in model.named_parameters():
            if param.requires_grad:
                rank0_print(
                    f"[DEBUG]Parameter Name: {name}, Shape: {param.shape}, grad: {param.requires_grad}")
    ...
    train_dataset_vlm = LazySupervisedDataset_stage3_vlm(
        args.data_path, tokenizer, data_args, args.imis_image_size,  mode='train')
    train_dataset_sam = LazySupervisedDataset_stage3_sam(
        args.data_path_seg, tokenizer, data_args, args.imis_image_size, mode='train')

    args.steps_per_epoch = math.ceil(math.ceil(2*max(len(
        train_dataset_vlm), len(train_dataset_sam)) / (args.batch_size * torch.cuda.device_count())) / args.grad_accumulation_steps)

    if args.no_eval == False:
        val_dataset = LazySupervisedDataset_stage3_vlm(
            args.val_data_path_seg, tokenizer, data_args, args.imis_image_size, mode='val')
    ...
    train_sampler_vlm = torch.utils.data.distributed.DistributedSampler(
        train_dataset_vlm, shuffle=True, drop_last=True
    )
    train_loader = torch.utils.data.DataLoader(
        ...
        sampler=train_sampler_vlm,
        collate_fn=partial(
            DataCollatorForSupervisedDataset_stage3_vlm,
        ),
    )
    train_sampler_sam = torch.utils.data.distributed.DistributedSampler(
        train_dataset_sam, shuffle=True, drop_last=True
    )
    train_loader_sam = torch.utils.data.DataLoader(
        train_dataset_sam,
        batch_size=args.batch_size,
        shuffle=False,
        num_workers=2,
        pin_memory=False,
        persistent_workers=True,
        sampler=train_sampler_sam,
        collate_fn=partial(
            DataCollatorForSupervisedDataset_stage3_sam,
        ),
    )
    ...
    train_sam_iter = iter(train_loader_sam)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        # train for one epoch
        train_iter, train_sam_iter, best_score, cur_ciou = train(
            train_loader,
            train_loader_sam,
            val_loader,
...
def train(
        ...
        for local_step in tqdm.tqdm(range(0, first_step_in_epoch)):
            for i in range(args.grad_accumulation_steps):
                if local_step % 2 == 0:
                    ...
                    try:
                        input_dict = next(train_sam_iter)
                    except:
                        train_sam_iter = iter(train_loader_sam)
                        input_dict = next(train_sam_iter)
    ...
    for local_step in range(first_step_in_epoch, args.steps_per_epoch):
        ...
        for i in range(args.grad_accumulation_steps):
            ...
            # ------------------ VLM to SAM ------------------
            if local_step % 2 == 0:
                set_trainable_parts(model, "vlm_sam", args)
                ...
                set_trainable_parts(model, "sam_vlm", args)

                imis_output_dict = model(mode="sam_vlm", **input_dict)
                loss1 = imis_output_dict["loss"]
            ...
            if local_step % 2 == 1:
                imis_total_loss = imis_output_dict["loss"]
                imis_ce_loss = imis_output_dict["ce_loss"]
                imis_mask_loss = imis_output_dict["mask_loss"]
                imis_total_losses.update(imis_total_loss.item(),
                                         input_dict["images_imis"].size(0))
                imis_ce_losses.update(imis_ce_loss.item(),
                                      input_dict["images_imis"].size(0))
                imis_mask_losses.update(imis_mask_loss.item(),
                                        input_dict["images_imis"].size(0))
                del imis_output_dict, imis_total_loss, imis_ce_loss, imis_mask_loss
        ...
        if local_step % args.print_freq == 0:
            ...
            if args.local_rank == 0:
                ...
                if local_step % 2 == 1:
                    writer.add_scalar(
                        "train/imis_total_loss", imis_total_losses.avg, model.global_steps)
                    writer.add_scalar(
                        "train/imis_ce_loss", imis_ce_losses.avg, model.global_steps)
                    writer.add_scalar(
                        "train/imis_mask_loss", imis_mask_losses.avg, model.global_steps)
                elif local_step % 2 == 0:
                    writer.add_scalar(
        ...
        if model.global_steps != 0:
            ...
            if args.local_rank == 0:
                ...
                writer.add_scalar(
                    "train/imis_lr", curr_lr[1], model.global_steps)
        ...
        if local_step != 0 and (local_step+1) % args.save_steps == 0:
        ...
        if local_step != 0 and (local_step+1) % 30 == 0:
    ...
    return train_iter, train_sam_iter, best_score, cur_ciou
...
def validate_vlm(val_loader, model_engine, tokenizer, writer, epoch, args, type="Object Hallucination"):
    ...
    for input_dict in pbar:
        ...
        with torch.no_grad():
            if args.proj_only:
                if args.precision == "fp16":
                    input_dict["images_clip_seg"] = input_dict["images_clip_seg"].half()
                    input_dict["images_imis_seg"] = input_dict["images_imis_seg"].half()
                elif args.precision == "bf16":
                    input_dict["images_clip_seg"] = input_dict["images_clip_seg"].bfloat16(
                    )
                    input_dict["images_imis_seg"] = input_dict["images_imis_seg"].bfloat16(
                    )
                else:
                    input_dict["images_clip_seg"] = input_dict["images_clip_seg"].float()
                    input_dict["images_imis_seg"] = input_dict["images_imis_seg"].float()
                output = model_engine.module.evaluate_sam_vlm(**input_dict)
                gt_value = normalize_word(
                    input_dict['answers_list_vlm'][0][0].lower())
